...
export function FileCard({ file, onDelete, onDownload, onPreview, isSelected, onClick, onContextMenu, onDrop, onDragStart, onDragEnd, activeFolderId, height, onToggleSelection, pathLabel, highlighted = false }: FileCardProps) {
        ...
        >
            ...
            >
                ...
                <div className={`absolute bottom-0 left-0 right-0 p-3 ${thumbnail ? 'text-white' : 'text-telegram-text'}`}>
                    ...
                    {pathLabel && (
                        <p className={`mt-0.5 truncate text-[11px] ${thumbnail ? 'text-white/60' : 'text-telegram-subtext/80'}`} title={pathLabel}>{pathLabel}</p>
                    )}
